...
impl OAuth2Flow {
    ...
    pub fn step_2_exchange_access_code(&self,
                                       access_code: &str)
                                       -> Result<AccessTokenData, OAuth2FlowError> {
        let client = Client::new();
        let url = try!(Url::parse(self.token_uri.as_str())
            .map_err(|_| OAuth2FlowError::InvalidUrlError));
        ...
        let mut res = try!(client.post(url)
            ...
            .body(body.as_str())
            .send()
            .map_err(|_| OAuth2FlowError::NetworkError));
        let mut res_body = String::new();
        try!(res.read_to_string(&mut res_body).map_err(|_| OAuth2FlowError::NetworkError));
        let result = try!(json::decode::<AccessTokenData>(res_body.as_str())
            .map_err(|_| OAuth2FlowError::JsonDecodeError));
        Ok(result)
    }
    ...
    pub fn refresh_access_token(&self,
        ...
        let body = format!("client_id={}&client_secret={}&refresh_token={}&grant_type={}",
                           self.client_id,
                           self.client_secret,
                           access_token_data.refresh_token,
                           "refresh_token");
        let mut res = try!(client.post(url)
            .headers(headers)
            .body(body.as_str())
        ...
        let mut res_body = String::new();
        try!(res.read_to_string(&mut res_body).map_err(|_| OAuth2FlowError::NetworkError));
        let result = try!(json::decode::<RefreshTokenData>(res_body.as_str())
            .map_err(|_| OAuth2FlowError::JsonDecodeError));
        Ok(AccessTokenData {
